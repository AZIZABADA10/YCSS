...
@section('content')
    <div class="page-header">
        <div>
            <h1 class="page-title">Modifier {{ $user->nom_complet }}</h1>
            <p class="page-subtitle">Modifier les informations du compte</p>
        </div>
    </div>
    ...
    <div class="card" style="max-width: 800px;">
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" enctype="multipart/form-data">
            ...
            <div style="text-align: center; margin-bottom: 24px;">
                <label for="photo" style="cursor: pointer; display: inline-block; position: relative;">
                    <div style="width: 100px; height: 100px; border-radius: 50%; background: #f1f5f9; border: 2px dashed #cbd5e1; display:flex; align-items:center; justify-content:center; overflow: hidden;">
                        @if($user->photo)
                            <img id="photo-preview" src="{{ asset('storage/' . $user->photo) }}" alt="Aperçu" style="width: 100%; height: 100%; object-fit: cover;">
                            <span id="photo-placeholder" style="color: #94a3b8; font-size: 24px; display: none;">+</span>
                        @else
                    ...
                    <div style="font-size: 12px; color: var(--text-gray); margin-top: 8px;">Modifier la photo</div>
                </label>
            ...
            <div class="form-row">
                <div class="form-col form-group">
                    <label class="form-label">Mot de passe (Laissez vide pour conserver l'actuel)</label>
                ...
                <div class="form-col form-group">
                    ...
                    <select name="statut" class="form-control" required>
                        <option value="1" {{ old('statut', $user->statut) == 1 ? 'selected' : '' }}>Actif</option>
                ...
                <div class="form-col form-group">
                    ...
                    <select name="role_id" class="form-control" required>
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" {{ old('role_id', $user->role_id) == $role->id ? 'selected' : '' }}>{{ $role->titre }}</option>
                        @endforeach
                    </select>
                    @error('role_id')
                        <span class="invalid-feedback">{{ $message }}</span>
